...
import {MODEL_CONFIG, IMAGE_CONFIG} from './PromptConstants';
...
export function Prompt() {
  ...
  async function handleSend() {
    ...
    try {
      ...
      // const prompt = prompts[detectType]; // Unused variable
      ...
      if (detectType === '3D bounding boxes') {
        ...
        model = MODEL_CONFIG.MODEL_3D;
      } else {
        // Disable thinking for 2.5 Flash, as recommended for spatial
        // understanding tasks.
        config['thinkingConfig'] = {thinkingBudget: MODEL_CONFIG.THINKING_BUDGET_DISABLED};
      }
        ...
        contents: [
          {
            ...
            parts: [
              {
                inlineData: {
                  data: activeDataURL.replace(IMAGE_CONFIG.DATA_PREFIX, ''),
      ...
      let response = generateResponse?.text ? generateResponse.text : '';
            ...
            {detectType === 'Segmentation masks' && (
              <>
                ...
                <textarea
                  ...
                  onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setSegmentationLanguage(e.target.value)}
                  onKeyDown={(e: React.KeyboardEvent<HTMLTextAreaElement>) => {
                    if (e.key === 'Enter' && !isLoading) {
                      e.preventDefault();
                      handleSend();
            ...
            {is2d && (
              <>
                ...
                <textarea
                  ...
                  value={labelPrompt}
                  onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setLabelPrompt(e.target.value)}
                  onKeyDown={(e: React.KeyboardEvent<HTMLTextAreaElement>) => {
                    if (e.key === 'Enter' && !isLoading) {
                      e.preventDefault();
                      handleSend();
